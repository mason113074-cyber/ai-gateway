...
export function registerProxyRoutes(
  ...
  app.all("/v1/*", async (req: unknown, reply: unknown) => {
    const req_ = req as { headers: Record<string, string | undefined>; params?: { "*"?: string }; method: string; raw?: { body?: { text: () => Promise<string> } } };
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const reply_ = reply as any;
    const startTime = Date.now();
    const agentId = req_.headers["x-agent-id"] ?? "anonymous";
    const teamId = req_.headers["x-team-id"] ?? "default";
    const provider = req_.headers["x-provider"] ?? "openai";
    ...
    if (req_.headers["x-api-key"]) headers["x-api-key"] = req_.headers["x-api-key"]!;
