...
- **Recent observability / proxy hardening**:
  ...
  - API entrypoint: `apps/api/src/server.ts` loads env and calls `startGatewayServer()` in `server-bootstrap.ts`; `/v1/*` is registered via `routes/register-proxy.ts` (`registerProxyPlugin`); REST handlers live under `apps/api/src/routes/*` and are aggregated in `routes/index.ts`.
