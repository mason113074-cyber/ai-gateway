...
export const POSTGRES_UNSUPPORTED_MESSAGE =
  "DATABASE_URL is set, but PostgreSQL runtime support is not available in this release. Remove DATABASE_URL and use DATABASE_PATH (SQLite).";

export type Database = ReturnType<typeof drizzleSqlite<typeof sqliteSchema>>;
export type RawDatabase = InstanceType<typeof BetterSqlite3>;

function initializeSqliteSchema(sqlite: RawDatabase): void {
  sqlite.exec(`
...
function assertPostgresDisabled(): void {
  if (process.env.DATABASE_URL) {
    throw new Error(POSTGRES_UNSUPPORTED_MESSAGE);
  }
}

export function createDatabase(dbPath: string = "./data/gateway.db"): Database {
  assertPostgresDisabled();
